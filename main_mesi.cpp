...
unsigned long long Bus, cache_read;
...
int Search_and_update(L2 *L2, char current_state, int op, unsigned long long set, unsigned long long tag, int cpu)
{
  ...
  int way, mod_found = 0, mod_way, mod_cpu, found = 0, shared_way, shared_cpu, shared_found, excl_found = 0, excl_way,excl_cpu ;
        ...
        else
        {
          ...
          //if conflict happens in L3 at way
          {
            ...
            for (int i = 0; i < CORE_COUNT; i++)
            {
              ...
              if (way != -1 && L2[threadid].dir[set][way] != I)
              {
